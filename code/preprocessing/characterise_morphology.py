...
from metadata.config import CODE_ROOT, MORPHOLOGY_CONFIG, PROJECT_ROOT
...
def main() -> None:
    ...
    write_manifest(SIZE_CSV, generating_script=f"code/preprocessing/{SCRIPT_NAME}.py")

    opening_table = pd.DataFrame(opening_rows)
    opening_table.to_csv(OPENING_CSV, index=False)
    write_manifest(OPENING_CSV, generating_script=f"code/preprocessing/{SCRIPT_NAME}.py")
