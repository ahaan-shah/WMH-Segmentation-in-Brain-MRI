...
# code/metadata/config.py -> parents[0] is metadata/, parents[1] is code/,
# parents[2] is the project root. Two roots, kept distinct on purpose:
#
#   PROJECT_ROOT  the repository. Holds data/, outputs/ (the figure gallery),
#                 reports/ and deliverables/ — things that are not source code.
#   CODE_ROOT     code/. Holds every importable package and the per-package
#                 outputs/ directory that sits beside its code.
#
# Anything a human reads (data, galleries, reports) hangs off PROJECT_ROOT;
# anything the pipeline writes beside its own source hangs off CODE_ROOT.
PROJECT_ROOT = Path(__file__).resolve().parents[2]
CODE_ROOT = PROJECT_ROOT / "code"

CONFIG_PATH = CODE_ROOT / "metadata" / "dataset.yaml"
...
HEAD_MASK_CONFIG = PREPROCESSING["head_mask"]
